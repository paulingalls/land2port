...
pub fn calculate_three_heads_crop(
    ...
    if use_stack_crop {
        ...
        if similar_size && equally_spaced {
            ...
            if left_area > right_area {
                ...
                let max_x = middle_head.xmax().max(right_head.xmax());
                ...
                return CropResult::Stacked(top_crop, bottom_crop);
            } else {
                // Top crop: two left heads (9:6)
                let min_x = left_head.xmin().min(middle_head.xmin());
                let max_x = left_head.xmax().max(middle_head.xmax());
                let center_between_two = (min_x + max_x) / 2.0;
            ...
            let all_heads_contained = heads.iter().all(|head| {
                ...
                let in_crop1 = head_xmin >= crop1_default.x
                    && head_xmax <= crop1_default.x + crop1_default.width;
                let in_crop2 = head_xmin >= crop2_default.x
                    && head_xmax <= crop2_default.x + crop2_default.width;
                in_crop1 || in_crop2
            });
            ...
            if all_heads_contained {
                ...
                let left_y =
                    vertical_y_for_heads(&left_heads, default_y, frame_height, crop_height);

                let right_y =
                    vertical_y_for_heads(&right_heads, default_y, frame_height, crop_height);
...
/// Calculates crop area for four or more heads
pub fn calculate_four_or_more_heads_crop(
    frame_width: f32,
    frame_height: f32,
    heads: &[&Hbb],
) -> CropResult {
    ...
    let has_dominant_head = head_areas.iter().enumerate().any(|(i, &area)| {
        head_areas
            .iter()
            .enumerate()
...
pub fn calculate_crop(
    ...
    match heads.len() {
        ...
        4.. => Ok(calculate_four_or_more_heads_crop(
            frame_width,
            frame_height,
            heads,
        )),
...
fn crop_result_distance(crop1: &CropResult, crop2: &CropResult) -> f32 {
    match (crop1, crop2) {
        (CropResult::Single(crop1), CropResult::Single(crop2)) => crop_area_distance(crop1, crop2),
        (CropResult::Stacked(crop1_1, crop1_2), CropResult::Stacked(crop2_1, crop2_2)) => {
        ...
        (CropResult::Resize(crop1), CropResult::Resize(crop2)) => crop_area_distance(crop1, crop2),
...
mod tests {
    ...
    fn test_calculate_four_and_five_heads_crop_close() {
        ...
        let crop = calculate_four_or_more_heads_crop(frame_width, frame_height, &heads);
    ...
    fn test_calculate_four_and_five_heads_crop_far_default() {
        ...
        match crop {
            ...
            _ => panic!("Expected single crop for far heads case"),
        }
    }
    ...
    fn test_calculate_four_and_five_heads_crop_all_heads_contained_with_vertical_adjust() {
        ...
        match crop {
            ...
            _ => panic!("Expected single crop for all_heads_contained with vertical adjust case"),
        }
    }
    ...
    fn test_calculate_three_heads_crop_fallback() {
        ...
        // Should fall back to the general stacked logic when special case doesn't apply
        match crop {
            CropResult::Stacked(crop1, crop2) => {
                let expected_width = frame_width * 0.5;
                let expected_height = expected_width * (8.0 / 9.0);

                // Both crops maintain the 8:9 aspect ratio from half-stack dims
                assert!((crop1.width - expected_width).abs() < 1.0);
                assert!((crop2.width - expected_width).abs() < 1.0);
                assert!((crop1.height - expected_height).abs() < 1.0);
                assert!((crop2.height - expected_height).abs() < 1.0);

                // Ensure crops remain within frame bounds
                assert!(crop1.x >= 0.0 && crop1.x + crop1.width <= frame_width);
                assert!(crop2.x >= 0.0 && crop2.x + crop2.width <= frame_width);
                assert!(crop1.y >= 0.0 && crop1.y + crop1.height <= frame_height);
                assert!(crop2.y >= 0.0 && crop2.y + crop2.height <= frame_height);

                // Validate head coverage: left head in first crop, others covered by second
                let head1_in_crop1 = head1.xmin() >= crop1.x && head1.xmax() <= crop1.x + crop1.width;
                assert!(head1_in_crop1, "First crop should contain left head");

                let head2_in_crop2 = head2.xmin() >= crop2.x && head2.xmax() <= crop2.x + crop2.width;
                let head3_in_crop2 = head3.xmin() >= crop2.x && head3.xmax() <= crop2.x + crop2.width;
                assert!(head2_in_crop2, "Second crop should contain middle head");
                assert!(head3_in_crop2, "Second crop should contain right head");
            }
            _ => panic!("Expected stacked crop for fallback case"),
        }
    }
    ...
    fn test_calculate_six_or_more_heads_crop_with_large_head() {
        ...
        match crop {
            ...
            _ => panic!("Expected single crop for large head case"),
        }
    }
